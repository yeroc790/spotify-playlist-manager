...
export async function getPlaylists(username, token) {
  let spotifyApi = getSpotifyApi(token)
  try {
    let data = await spotifyApi.getUserPlaylists(username)
    return data.body
  } catch (error) {
...
export async function createPlaylist(name, description, isPrivate, token) {
  let spotifyApi = getSpotifyApi(token)

  try {
    await spotifyApi.createPlaylist(name, { 'description': description, 'public': isPrivate })
  } catch (error) {
    console.error('Something went wrong! ', error)
    throw new Error('Something went wrong! ', error)
  }
...
export async function searchSpotify(str, type, token) {
  ...
  try {
    let data = await spotifyApi.search(str, [type])
    return data.body
  } catch (error) {
    console.error('Something went wrong! ', error)
    throw new Error('Something went wrong! ', error)
