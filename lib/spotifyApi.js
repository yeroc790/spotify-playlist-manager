...
export async function getPlaylists(username, offset, token) {
  let spotifyApi = getSpotifyApi(token)
  try {
    let data = await spotifyApi.getUserPlaylists(username, {offset: offset})
    return data.body
  } catch (error) {
...
export async function getPlaylistTracks(playlistID, offset, token) {
  let spotifyApi = getSpotifyApi(token)

  try {
    let data = await spotifyApi.getPlaylistTracks(playlistID, {offset: offset})
    return data.body
  } catch (error) {
    console.log('Something went wrong! ', error)
    throw new Error("Something went wrong! ", error)
...
export async function getPlaylistUris(playlistID, offset, token) {
  let spotifyApi = getSpotifyApi(token)
  let options = {
    offset: offset,
...
export async function getArtistAlbums(artistID, token) {
  ...
  try {
    let data = await spotifyApi.getArtistAlbums(artistID, {market: 'from_token'})
    return data.body
  } catch (error) {
    console.log('Something went wrong! ', error)
    throw new Error("Something went wrong! ", error)
...
export async function getAlbumTracks(albumID, token) {
  ...
  try {
    let data = await spotifyApi.getAlbumTracks(albumID, [50])
    return data.body
  } catch (error) {
    console.log('Something went wrong! ', error)
    throw new Error("Something went wrong! ", error)
...
export async function editPlaylist(playlistId, name, description, isPublic, token) {
  ...
  try {
    // await spotifyApi.changePlaylistDetails(playlistId, { 'name': name, 'description': description, 'public': isPublic })
